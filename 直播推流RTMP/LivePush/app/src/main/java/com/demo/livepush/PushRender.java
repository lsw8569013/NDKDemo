...
public  class PushRender implements GLSurfaceView.Renderer {
    ...
    public PushRender(Context context, int textureId) {
        ...
        this.mCameraTextureId = textureId;
...
//        mFboRender.onSurfaceCreated(mViewWidth, mViewHeight);
        ...
        if(filterId ==-1){
             fragmentSource = Utils.getGLResource(mContext, R.raw.fragment_shader);
        }else{
             fragmentSource = Utils.getGLResource(mContext, filterId);
        }
...
//        GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_BINDING_EXTERNAL_OES, mCameraTextureId);
//        // 设置纹理环绕方式
...
//
//        mCameraSt = new SurfaceTexture(mCameraTextureId);
//
...
//        Matrix.rotateM(matrix, 0, 180, 1, 0, 0);
    ...
    public void setFragmentRender(int filter_fragment_gray) {
       this. filterId = filter_fragment_gray;
    }
